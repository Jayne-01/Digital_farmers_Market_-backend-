...
class User {
    static async create(userData) {
        const { full_name, email, password, role, contact_number, address } = userData;
        const query = `
            INSERT INTO users (full_name, email, password, role, contact_number, address)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING user_id, full_name, email, role, contact_number, address, created_at
        `;
        return await db.query(query, [full_name, email, password, role, contact_number, address]);
    }
