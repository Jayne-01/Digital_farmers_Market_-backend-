        ...
        if (user.role === 'FARMER') {
            ...
            if (farmerResult.rows.length > 0) {
                ...
                const statsQuery = `
                    ...
                    WHERE f.farmer_id = $1
                `;
                const statsResult = await db.query(statsQuery, [additionalInfo.farmer_profile.farmer_id]);
            ...
            const customerQuery = `
                ...
                GROUP BY u.user_id
            `;
            const customerResult = await db.query(customerQuery, [user_id]);
...
const updateUserRole = async (req, res) => {
    try {
        ...
        if (currentUser.role === 'FARMER' && role !== 'FARMER') {
            ...
            await db.query('DELETE FROM farmers WHERE user_id = $1', [user_id]);
...
const verifyFarmer = async (req, res) => {
    try {
        ...
        const result = await db.query(updateQuery, [verified_status, farmer_id]);
...
const updateOrder = async (req, res) => {
    try {
        ...
        const result = await db.query(query, [order_id, ...values]);
